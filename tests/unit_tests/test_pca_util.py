...
class TestPCAUtils(TestCase):
    ...
    def test_train_pca_dask(self):
        ...
        strel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, tuple(config_data["tailfilter_size"])
        )
        tailfilter = strel
        ...
        for fp in h5ps:
            try:
                ...
                assert (
                    False
                ), "h5 File pointer is still open"  # added assertion here to fail test in case file is still open
    ...
    def test_apply_pca_local(self):
        ...
        use_fft, clean_params, mask_params, missing_data = get_pca_yaml_data(
            f"{pca_path}.yaml"
        )
    ...
    def test_get_changepoints_dask(self):
        ...
        get_changepoints_dask(
            ...
            client,
            30,
            pca_scores=f"{missing_data_save_file}.h5",
        )
        client.restart()
        client.close()
